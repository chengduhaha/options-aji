...
export default function CongressPage() {
    ...
    try {
      const res = await fetch("/api/congress/backtest", {
        ...
        body: JSON.stringify({
          member: btMember,
          chamber: btChamber,
          capital: parseFloat(btCapital) || 100_000,
        }),
      });
      if (!res.ok) {
  ...
  function TxBadge({ type }: { type: string }) {
    ...
    return (
      <span className="px-1.5 py-0.5 rounded text-[9px] font-bold bg-glass text-muted-foreground border border-glass-border">
        {type}
      </span>
    );
  }
  ...
  const TABS: { id: Tab; label: string }[] = [
    ...
    { id: "leaderboard", label: "领袖榜" },
    { id: "backtest", label: "回测工具" },
  ];
      ...
      {tab === "trades" && (
        <div className="space-y-4">
          <div className="flex flex-wrap gap-3 items-center">
            ...
            <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-glass border border-glass-border">
              ...
              <input
                ...
                className="bg-transparent text-sm text-foreground placeholder:text-muted outline-none w-28"
              />
            </div>
            <button
              onClick={fetchTrades}
              disabled={tradesLoading}
              className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-glass border border-glass-border text-sm text-muted-foreground hover:text-foreground disabled:opacity-50"
            >
              <RefreshCw
                className={clsx("w-3.5 h-3.5", tradesLoading && "animate-spin")}
              />
              搜索
            </button>
          </div>
          ...
          <div className="rounded-xl border border-glass-border bg-glass/40 overflow-hidden">
            {tradesError && (
              <div className="p-4 text-center text-red-400 text-sm">{tradesError}</div>
            )}
            {tradesLoading && (
                  ...
                  {trades.map((trade) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <ChamberBadge c={trade.chamber} />
                      </td>
                      <td className="px-4 py-3 font-bold text-foreground font-mono">
                        {trade.symbol}
                      </td>
                      <td className="px-4 py-3">
                        <TxBadge type={trade.transaction_type} />
                      </td>
      ...
      {tab === "leaderboard" && (
        <div className="rounded-xl border border-glass-border bg-glass/40 overflow-hidden">
          ...
          {!lbLoading && lb.length === 0 && (
            <div className="p-8 text-center text-muted-foreground text-sm">暂无排行榜数据</div>
          )}
          {!lbLoading && lb.length > 0 && (
            <table className="w-full text-sm">
              <thead>
              ...
              <tbody>
                {lb.map((entry, idx) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <ChamberBadge c={entry.chamber} />
                    </td>
                    <td className="px-4 py-3 text-right">
                      <span
                        className={clsx(
                          "flex items-center justify-end gap-1 font-bold",
                          entry.hypothetical_roi_pct >= 0
                            ? "text-green-400"
                            : "text-red-400",
                        )}
                      >
                        ...
                        {entry.hypothetical_roi_pct >= 0 ? "+" : ""}
                        {entry.hypothetical_roi_pct?.toFixed(1)}%
                      </span>
                    </td>
                    <td className="px-4 py-3 text-right text-muted-foreground">
                      {entry.trade_count}
                    </td>
                    <td className="px-4 py-3 font-mono font-bold text-foreground text-[12px]">
                      {entry.best_trade_symbol}
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <div className="rounded-xl border border-glass-border bg-glass/40 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
              <div>
                ...
                <input
                  type="text"
                  placeholder="例如: Nancy Pelosi"
                  value={btMember}
                  onChange={(e) => setBtMember(e.target.value)}
                  className="w-full px-3 py-2 rounded-lg bg-glass border border-glass-border text-sm text-foreground placeholder:text-muted outline-none focus:border-primary/50"
                />
              </div>
              <div>
                <label className="block text-[11px] text-muted uppercase tracking-wide mb-1.5">
                ...
                <select
                  value={btChamber}
                  onChange={(e) => setBtChamber(e.target.value)}
            ...
            {btError && (
              <p className="text-red-400 text-sm mb-3">{btError}</p>
            )}
            <button
              onClick={runBacktest}
              disabled={btLoading}
              className="flex items-center gap-2 px-6 py-2.5 rounded-xl bg-primary/20 border border-primary/30 text-primary font-medium text-sm hover:bg-primary/30 transition-all disabled:opacity-50"
            >
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                  {
                    label: "初始资金",
                    value: `$${btResult.initial_capital.toLocaleString()}`,
                    color: "text-foreground",
                  },
                  {
                    label: "最终价値",
                    value: `$${btResult.final_value.toLocaleString()}`,
                    color: "text-foreground",
                  },
                  {
                    label: "总收益率",
                    value:
                      (btResult.total_return_pct >= 0 ? "+" : "") +
                      btResult.total_return_pct?.toFixed(2) +
                      "%",
                    color:
                      btResult.total_return_pct >= 0
                        ? "text-green-400"
                  ...
                  },
                  {
                    label: "胜率",
                    value:
                      btResult.trade_count > 0
                        ? `${((
                            btResult.winning_trades / btResult.trade_count
                          ) *
                            100
                          ).toFixed(0)}%`
                        : "N/A",
                  ...
                  <div
                    key={label}
                    className="rounded-xl border border-glass-border bg-glass/40 p-4"
                  >
                  ...
                  <table className="w-full text-sm">
                      ...
                      <tr className="border-b border-glass-border">
                        {[
                          ...
                          <th
                            key={label}
                            className={clsx(
                              "px-4 py-2 text-[10px] font-semibold text-muted uppercase",
                              align,
                            )}
                          >
                            {label}
                    ...
                    <tbody>
                      {btResult.trades.map((t, i) => (
                        <tr
                          key={i}
                          className="border-b border-glass-border/50 hover:bg-glass/60"
                        >
